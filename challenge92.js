...
const twoSum = function (nums, target) {
  const store = {}; // Stores seen numbers: {seenNumber: indexItOccurred}
  ...
  for (let i = 0; i < nums.length; i++) {
    ...
    if (store[target - n] !== undefined) {
      ...
      return [store[target - n], i]; // grab the index of the seen number, and the index of the current number
    }
    store[n] = i; // update ourstore to include the. number we just saw along with its index.
  }
